<?php
 ...
 if ($usuario1=="rouget") {$gab="%";}
 if ($usuario1=="cadu") {$gab="%";}
 if ($usuario1=="pita") {$gab="%";}
 if ($usuario1=="carla") {$gab="%"; }
 if ($usuario1=="cunha") {$gab="%";}
 if ($usuario1=="josefina") {$gab="%";}
 if ($usuario1=="santana") {$gab="%";}
 if ($usuario1=="altino") {$gab="1";}
 if ($usuario1=="cesaltina") {$gab="1";}
 if ($usuario1=="tito") {$gab="3";}
 if ($usuario1=="faustino") {$gab="4";}
 if ($usuario1=="george") {$gab="4";}
 if ($usuario1=="almeida") {$gab="4";}
 if ($usuario1=="montero") {$gab="4";}
 if ($usuario1=="odete") {$gab="4";}
 if ($usuario1=="manuel") {$gab="4";}
 if ($usuario1=="ezequiel") {$gab="5";}
...
# require ('liga2.php');
 ...
 $sql = mysqli_query($conexion, "SELECT * FROM reclamaciones t1 INNER JOIN estudantes t2 ON t1.processo=t2.processo WHERE (confirma LIKE '$confirma' && curso='$curso' && gabinetes LIKE '$gab') && (data BETWEEN '$data01' AND '$data02')"); 
...
if ($confirma==1)
     {
   $arreglo = array("Código", "Curso", "Processo", "Telemóvel", "Reclamação", "Data Envío", "DataDespacho", "Despacho", "Despachado Por:");
   $cantidad = array_count_values($arreglo);
  $conta = "9";
     }
    else
    {
   $arreglo = array("Código", "Curso", "Processo", "Telemóvel", "Reclamação", "Data Envío");
   $cantidad = array_count_values($arreglo);
  $conta = "6";
     }
    
  for ($roda=0; $roda<$conta; $roda++)
        {
 ...
 #    echo "<th style=text-align:center  ><H6><pre><font face=arial>&nbsp;".wordwrap($user,$largoMax,$rompeLineas,$romper_palabras_largas)."&nbsp;</font></pre></H6></th> \n";
