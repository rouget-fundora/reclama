...
mysqli_select_db($conexion, "reclama");

 $sql = mysqli_query($conexion, "SELECT * FROM reclamaciones WHERE id LIKE '$code1'"); 
 
 if ($row = mysqli_fetch_array($sql)){

   mysqli_field_seek($sql,0);
   
   $processo = $row["processo"];
   $reclama = $row["reclama"];
	
   $SQL1 = mysqli_query($conexion,"INSERT INTO apagar (usuario,codigo,processo,reclama) VALUES ('$usuario2','$code1','$processo','$reclama')");

   $SQL2 = mysqli_query($conexion, "DELETE FROM reclamaciones WHERE id = $code1");

   echo "<div align=center>";
       echo '</br>';
       echo "Reclamação -> ".$code1.", Apagada !!";
   echo "</div>";
...

      {
        ...
        echo "<div align=center>";
           echo '</br>';
           echo "Reclamação -> ".$code1.", Não Exite !!";
        echo "</div>";
